 ...
 * @LastEditors: lax
 * @LastEditTime: 2021-04-13 15:43:32
...
function getRow(x, y) {
	...
	row.fill(new Element());
	row[getNumByEachBoxOnlyHaveOne(x, y)].activate();
	row[getNumByEachRowMustHaveOneToNine(x, y)].activate();
	row[getNumByEachColMustHaveOneToNine(x, y)].activate();
	row[getNumByEachPalaceMustHaveOneToNine(x, y)].activate();
	const result = { x, y, row };
	return result;
}
